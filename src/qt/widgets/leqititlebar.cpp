...
void LeqiTitleBar::initialTitleBar()
{
    ...
    LeqiButton *leqiButton = new LeqiButton(TIP_APPMENU, OBJ_NAME_LEQI, APP_NAME);
    ...
    labelTitle->setObjectName(OBJ_NAME_TITLE);
    titleBarLayout->addWidget(labelTitle);
    titleBarLayout->addStretch(1);
    LeqiButton *fullScreenButton = new LeqiButton(TIP_FULLSCREEN, OBJ_NAME_FULLSCREEN);
    LeqiButton *minimizeButtton = new LeqiButton(TIP_MINIMIZE, OBJ_NAME_MINIMIZE);
    LeqiButton *closeButton = new LeqiButton(TIP_CLOSE, OBJ_NAME_CLOSE);
