...
void LeqiYingYin::initMainWindow()
{
    ...
    loadStyleSheet(QSS_DARK);
    setWindowIcon(QIcon(ICON_LEQI));
...
{
    QString fileName = QFileDialog::getOpenFileName(this, TIP_OPEN
                                , tr("D:/shipin"), tr("*.*"));
    if (fileName.isEmpty())
    {
        return;
...
void LeqiYingYin::toggleMute()
{
    ...
    {
        emit setVoiceIcon(ICON_MUTE, TIP_CANCEL_MUTE);
        return;
    }

    emit setVoiceIcon(ICON_VOICE, TIP_MUTE);
}
